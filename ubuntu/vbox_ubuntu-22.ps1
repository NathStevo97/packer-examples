...
$var_file="./variables/ubuntu-22.pkrvars.hcl"
$machine="ubuntu 22"
$packer_log=0
...
if ((Test-Path -Path "$template_file")) {
  ...
  try {
    ...
    packer validate -var-file="$var_file" -only='virtualbox-iso.ubuntu' "$template_file"
  }
  catch {
  ...
  try {
    $env:PACKER_LOG=$packer_log
    packer version
    packer build -var-file="$var_file" -only='virtualbox-iso.ubuntu' --force "$template_file"
  }
  catch {

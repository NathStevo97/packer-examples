...
if ((Test-Path -Path "$template_file") -and (Test-Path -Path "$var_file")) {
  ...
  try {
    ...
    packer validate -var-file="$var_file" -only='hyperv-iso.hv2-win2022-standard' -var-file="$var_file" "$template_file"
  }
  catch {
  ...
  try {
    $env:PACKER_LOG=$packer_log
    packer version
    packer build -var-file="$var_file" -only='hyperv-iso.hv2-win2022-standard' --force -var-file="$var_file" "$template_file"
  }
  catch {

...
$machine="Windows 10"
$packer_log=1
$env:PACKER_LOG_PATH="packerlog-10-vmware.txt"
...
if ((Test-Path -Path "$template_file")) {
  ...
  try {
    ...
    packer validate -var-file="$var_file" -only='vmware-iso.vmware-win10' "$template_file"
  }
  catch {
  ...
  try {
    ...
    packer build -var-file="$var_file" -only='vmware-iso.vmware-win10' --force "$template_file"
  }
  catch {

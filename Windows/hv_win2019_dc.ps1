...
if ((Test-Path -Path "$template_file")) {
  ...
  try {
    ...
    packer validate -var-file="$var_file" -only='hyperv-iso.hv1-win2019-datacenter' "$template_file"
  }
  catch {
  ...
  try {
    ...
    packer build -var-file="$var_file" -only='hyperv-iso.hv1-win2019-datacenter' --force "$template_file"
  }
  catch {

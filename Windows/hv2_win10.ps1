...
$machine="Windows 10"
$packer_log=0
$env:PACKER_LOG_PATH="packerlog-10-hv2.txt"
...
if ((Test-Path -Path "$template_file")) {
  ...
  try {
    ...
    packer validate -var-file="$var_file" -only='hyperv-iso.hv2-win10' "$template_file"
  }
  catch {
  ...
  try {
    ...
    packer build -var-file="$var_file" -only='hyperv-iso.hv2-win10' --force "$template_file"
  }
  catch {

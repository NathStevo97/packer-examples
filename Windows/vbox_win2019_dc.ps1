...
$packer_log=0
$env:PACKER_LOG_PATH="packerlog-2019-DC-vbox.txt"
...
if ((Test-Path -Path "$template_file")) {
  ...
  try {
    ...
    packer validate -var-file="$var_file" -only='virtualbox-iso.vbox-win2019-dc' "$template_file"
  }
  catch {
  ...
  try {
    ...
    packer build -var-file="$var_file" -only='virtualbox-iso.vbox-win2019-dc' --force "$template_file"
  }
  catch {

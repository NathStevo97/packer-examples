...
if ((Test-Path -Path "$template_file")) {
  ...
  try {
    ...
    packer validate -var-file="$var_file" -only='virtualbox-iso.vbox-win10' "$template_file"
  }
  catch {
  ...
  try {
    ...
    packer build -var-file="$var_file" -only='virtualbox-iso.vbox-win10' -force "$template_file"
  }
  catch {

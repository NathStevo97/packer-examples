...
# Variables
$template_file="./templates/almalinux.pkr.hcl"
...
if ((Test-Path -Path "$template_file")) {
  ...
  try {
    ...
    packer validate -only='vmware-iso.almalinux-9' "$template_file"
  }
  catch {
  ...
  try {
    ...
    packer build -only='vmware-iso.almalinux-9' --force "$template_file"
  }
  catch {

...
# Variables
$env:PACKER_LOG_PATH="packerlog-fedora-$Version.txt"
packer init "../required_plugins.pkr.hcl"

$machine="Fedora Linux $Version"
...
if ((Test-Path -Path "$template")) {
  ...
  try {
    ...
    packer validate -var-file="$var_file" -only="$Provider.fedora" "$template"
  }
  catch {
  ...
  try {
    $env:PACKER_LOG=$packer_log
    packer version
    packer build -var-file="$var_file" -only="$Provider.fedora" --force "$template"
  }
  catch {

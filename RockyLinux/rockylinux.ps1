...
# Variables
$env:PACKER_LOG_PATH="packerlog-rockylinux-$Version.txt"
packer init -upgrade "../required_plugins.pkr.hcl"

$machine="Rocky Linux $Version"
...
if ((Test-Path -Path "$template")) {
  ...
  try {
    ...
    packer validate -var-file="$var_file" -only="$Provider.rockylinux" "$template"
  }
  catch {
  ...
  try {
    $env:PACKER_LOG=$packer_log
    packer version
    packer build -var-file="$var_file" -only="$Provider.rockylinux" --force "$template"
  }
  catch {

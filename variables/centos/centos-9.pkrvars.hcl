...
boot_command_hyperv      = ["c  setparams 'kickstart' <enter> linuxefi /images/pxeboot/vmlinuz inst.repo=http://mirror.stream.centos.org/9-stream/BaseOS/x86_64/os/ inst.addrepo= inst.stage2=hd:LABEL=CentOS-9-x86_64-dvd inst.text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/centos/ks-9-stream.cfg<enter> initrdefi /images/pxeboot/initrd.img<enter> boot<enter>"]
boot_wait                = "5s"
disk_size                = "40960"

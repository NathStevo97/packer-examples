...
boot_command_hyperv  = ["c  setparams 'kickstart' <enter> linuxefi /images/pxeboot/vmlinuz inst.stage2=hd:LABEL=Fedora-40-x86_64-dvd inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/fedora/ks-40.cfg<enter> initrdefi /images/pxeboot/initrd.img inst.repo=https://gb.mirrors.cicku.me/fedora/linux/releases/40/Server/x86_64/os/<enter> boot<enter>"]
boot_wait            = "5s"
disk_size            = "40960"
headless             = false

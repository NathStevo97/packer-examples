...
param(
    ...
    [ValidateSet("vmware-iso", "hyperv-iso")]
    [string]$Provider = ""
)
...
if ((Test-Path -Path "$template")) {
  ...
  try {
    ...
    packer validate -var-file="$var_file" -only="$Provider.oracle" "$template"
  }
  catch {
  ...
  try {
    $env:PACKER_LOG=$packer_log
    packer version
    packer build -var-file="$var_file" -only="$Provider.oracle" --force "$template"
  }
  catch {

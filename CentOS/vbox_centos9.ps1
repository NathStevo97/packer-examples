...
if ((Test-Path -Path "$template_file")) {
  ...
  try {
    ...
    packer validate -only='virtualbox-iso.centos-stream-9_vbox' -var-file="$var_file" "$template_file"
  }
  catch {
  ...
  try {
    $env:PACKER_LOG=$packer_log
    packer version
    packer build -only='virtualbox-iso.centos-stream-9_vbox' -var-file="$var_file" -force "$template_file"
  }
  catch {

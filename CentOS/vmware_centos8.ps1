...
if ((Test-Path -Path "$template_file")) {
  ...
  try {
    ...
    packer validate -only='vmware-iso.centos8_vmware' -var-file="$var_file" "$template_file"
  }
  catch {
  ...
  try {
    ...
    packer build -only='vmware-iso.centos8_vmware' -var-file="$var_file" --force "$template_file"
  }
  catch {
